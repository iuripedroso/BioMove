 ...
 * (`GeneralHealthDetailPage`) consiga montar o formulário e mostrar o resultado de
 * qualquer um dos 8 testes, sem precisar de uma página dedicada por teste.
 */

export interface GeneralHealthTestFieldOption {
  value: string | number;
  label: string;
}

export interface GeneralHealthTestField {
  /** Chave usada no formulário e repassada para o GeneralHealthTestsService.calculate(). */
  key: string;
  label: string;
  type: 'number' | 'select';
  unit?: string;
  options?: GeneralHealthTestFieldOption[];
  min?: number;
  max?: number;
  step?: number;
...
export interface GeneralHealthTestConfig {
  id: string;
  ...
  imageUrl?: string;
  fields: GeneralHealthTestField[];
}

export interface GeneralHealthTestResultRow {
  label: string;
  value: string;
}

export interface GeneralHealthTestResult {
  rows: GeneralHealthTestResultRow[];
}

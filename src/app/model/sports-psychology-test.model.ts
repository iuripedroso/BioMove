 ...
 * (`SportsPsychologyDetailPage`) consiga montar o formulário e mostrar o resultado de
 * qualquer um dos 8 testes, sem precisar de uma página dedicada por teste.
 */

export interface SportsPsychologyTestFieldOption {
  value: string | number;
  label: string;
}

export interface SportsPsychologyTestField {
  /** Chave usada no formulário e repassada para o SportsPsychologyTestsService.calculate(). */
  key: string;
  label: string;
  type: 'number' | 'select';
  unit?: string;
  options?: SportsPsychologyTestFieldOption[];
  min?: number;
  max?: number;
  step?: number;
...
export interface SportsPsychologyTestConfig {
  id: string;
  ...
  imageUrl?: string;
  fields: SportsPsychologyTestField[];
}

export interface SportsPsychologyTestResultRow {
  label: string;
  value: string;
}

export interface SportsPsychologyTestResult {
  rows: SportsPsychologyTestResultRow[];
}

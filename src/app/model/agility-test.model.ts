 ...
 * (`AgilityDetailPage`) consiga montar o formulário e mostrar o resultado de
 * qualquer um dos 8 testes, sem precisar de uma página dedicada por teste.
 */

export interface AgilityTestFieldOption {
  value: string | number;
  label: string;
}

export interface AgilityTestField {
  /** Chave usada no formulário e repassada para o AgilityTestsService.calculate(). */
  key: string;
  label: string;
  type: 'number' | 'select';
  unit?: string;
  options?: AgilityTestFieldOption[];
  min?: number;
  max?: number;
  step?: number;
...
export interface AgilityTestConfig {
  id: string;
  ...
  analysisNote: string;
  fields: AgilityTestField[];
}

export interface AgilityTestResultRow {
  label: string;
  value: string;
}

export interface AgilityTestResult {
  rows: AgilityTestResultRow[];
}

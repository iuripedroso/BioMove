...
} from 'src/app/model/agility-test.model';
...
@Component({
  selector: 'app-agility-detail',
  templateUrl: './agility-detail.page.html',
  styleUrls: ['./agility-detail.page.scss'],
})
export class AgilityDetailPage implements OnInit {
  test?: AgilityTestConfig;
  form: FormGroup = this.formBuilder.group({});
  resultRows: AgilityTestResultRow[] | null = null;
  ...
  ngOnInit() {
    ...
    this.test = this.agilityTestsService.getTests().find((t) => t.id === testId);
  ...
  private buildForm(test: AgilityTestConfig) {
  ...
  repeatControls(field: AgilityTestField): FormArray {
  ...
  calcular() {
    ...
    const result = this.agilityTestsService.calculate(this.test.id, this.form.value);

...
} from 'src/app/model/mobility-test.model';
...
@Component({
  ...
  templateUrl: './mobility-detail.page.html',
  styleUrls: ['./mobility-detail.page.scss'],
})
export class MobilityDetailPage implements OnInit {
  test?: MobilityTestConfig;
  form: FormGroup = this.formBuilder.group({});
  resultRows: MobilityTestResultRow[] | null = null;
  ...
  ngOnInit() {
    ...
    this.test = this.mobilityTestsService.getTests().find((t) => t.id === testId);
  ...
  private buildForm(test: MobilityTestConfig) {
  ...
  repeatControls(field: MobilityTestField): FormArray {
  ...
  calcular() {
    ...
    const result = this.mobilityTestsService.calculate(this.test.id, this.form.value);
